...
public class TaxPage extends OutlinePage {
    ...
    private VBox buildCenter() {
        ...
        TaxViewModel.totalPagesProperty().addListener((_, _, _) -> {
            if (TaxViewModel.getTotalPages() > 0) {
                paging.setVisible(true);
                paging.setManaged(true);
            } else {
    ...
    private void showDialog() {
        ...
        dialog.getChildren().add(new TaxForm(modalPane));
    ...
    private ContextMenu showContextMenu(TableRow<Tax> obj) {
        ...
        edit.setOnAction(
                event -> {
                    TaxViewModel.getTax(obj.getItem().getId(), this::showDialog, this::errorMessage);
                    event.consume();
                });
    ...
    private void setupTableColumns() {
        ...
        percentage.setCellFactory(_ -> new TableCell<>() {
            @Override
            public void updateItem(Tax item, boolean empty) {
                super.updateItem(item, empty);
