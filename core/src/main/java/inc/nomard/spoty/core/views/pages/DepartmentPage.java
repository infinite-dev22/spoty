...
public class DepartmentPage extends OutlinePage {
    ...
    private VBox buildCenter() {
        ...
        DepartmentViewModel.totalPagesProperty().addListener((_, _, _) -> {
            if (DepartmentViewModel.getTotalPages() > 0) {
                paging.setVisible(true);
                paging.setManaged(true);
            } else {
    ...
    private ContextMenu showContextMenu(TableRow<Department> obj) {
        ...
        delete.setOnAction(event -> new DeleteConfirmationDialog(AppManager.getGlobalModalPane(), () -> {
            DepartmentViewModel.deleteItem(obj.getItem().getId(), this::onSuccess, SpotyUtils::successMessage, this::errorMessage);
            event.consume();
        }, obj.getItem().getName()).showDialog());
        // Edit
        edit.setOnAction(
                e -> {
                    DepartmentViewModel.getItem(obj.getItem().getId(), this::showDialog, this::errorMessage);
                    e.consume();
                });
    ...
    private void showDialog() {
        ...
        dialog.getChildren().add(new DepartmentForm(modalPane));
