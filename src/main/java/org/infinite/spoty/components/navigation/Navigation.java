...
public class Navigation {
    ...
    public static Map<String, NavTree.NavTreeItem> createNavItems() {
        ...
        map.put("EXPENSECATEGORY", NavTree.NavTreeItem.page("Category", Pages.getExpenseCategoryPane()));
    ...
    private NavTree.NavTreeItem createTree() {
        var dashboard = NavTree.NavTreeItem.mainPage("Dashboard", "fas-chart-simple", Pages.getDashboardPane());

        var inventory = NavTree.NavTreeItem.group("Inventory", "fas-cubes");
        inventory.getChildren().setAll(
        ...
        var requisition = NavTree.NavTreeItem.mainPage("Requisitions", "fas-hand-holding", Pages.getRequisitionPane());
        var purchase = NavTree.NavTreeItem.mainPage("Purchases", "fas-cart-plus", Pages.getPurchasePane());
        var transfer = NavTree.NavTreeItem.mainPage("Transfers", "fas-arrow-right-arrow-left", Pages.getTransferPane());
        var stockIn = NavTree.NavTreeItem.mainPage("Stock In", "fas-cart-flatbed", Pages.getStockInPane());
        ...
        returns.getChildren().setAll(
                NAV_TREE.get("SALERETURN"),
                NAV_TREE.get("PURCHASERETURN")
        );

        var expense = NavTree.NavTreeItem.group("Expenses", "fas-wallet");
        expense.getChildren().setAll(
                NAV_TREE.get("EXPENSECATEGORY"),
                NAV_TREE.get("EXPENSE")
        );

        var people = NavTree.NavTreeItem.group("People", "fas-users");
        people.getChildren().setAll(
                NAV_TREE.get("CUSTOMER"),
                NAV_TREE.get("SUPPLIER"),
                NAV_TREE.get("USER")
        );

        var root = NavTree.NavTreeItem.root();
        root.getChildren().addAll(dashboard, inventory, requisition, purchase, transfer, stockIn, sale, returns, expense, people);
        return root;
    }
    ...
    private void loadView(BorderPane view) {
        try {
            final BorderPane prevWindow = (BorderPane) viewWindow.getChildren().stream()
                    .filter(c -> c instanceof BorderPane)
                    .findFirst()
                    .orElse(null);
            final BorderPane nextWindow = view;
            BorderPane existingNextChild = (BorderPane) viewWindow.getChildren().stream().findAny().filter(c -> c.equals(view)).orElse(null);

            if (viewWindow.getChildren().isEmpty()) {
                viewWindow.getChildren().add(nextWindow);
                return;
            } else if (!Objects.equals(existingNextChild, null)) {
                viewWindow.getChildren().get((int) existingNextChild.getViewOrder()).toFront();
            }
