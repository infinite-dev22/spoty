...
public class SaleReturnMasterDao {
  ...
  public static void updateSaleReturnMaster(SaleReturnMaster obj, int id) {
    ...
    try (Session session = HibernateUtil.getSessionFactory().openSession()) {
      ...
      obj.getSaleReturnDetails()
          .forEach(
              saleReturnDetail -> {
                if (saleReturnDetail.getSaleReturn() == null)
                  saleReturnDetail.setSaleReturn(saleReturnMaster);
              });
    ...
    try (Session session = HibernateUtil.getSessionFactory().openSession()) {
      ...
      saleCategories =
          FXCollections.observableList(
              session.createQuery("from SaleReturnMaster", SaleReturnMaster.class).stream()
                  .toList());
      transaction.commit();
    } catch (HibernateError ex) {
      if (transaction != null) transaction.rollback();
